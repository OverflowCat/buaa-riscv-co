...
`sra` 指令的格式为 ```asm sra rd, rs1, rs2```，即将寄存器 `rs1` 的值右移 `rs2` 位后存入寄存器 `rd`。因此，该指令的汇编形式为 *```asm sra x3, x1, x2```*，功能为*将寄存器 `x1` 的值右移 #fix("x")2 位后存入寄存器 `x3`，#fix[空位用 `x1` 最高位填充]*。
...
- `lw` 为 I 型指令，opcode 为 `0b0000011`，funct3 为 `0b010`；
- rs1 $= 22 = 010110_(2)$，imm $= 16 = 00000010000_(2)$，rd $= 9 = 1001_(2)$。

故该机器指令*二进制编码为 `00000010000 010110 010 01001 0000011`，十六进制编码为 `0x20b2483`*。

`lw` 的描述为 `rd = M[rs1+imm][0:31]`，此处为 `x9 = M[x22+32][0:31]`，即该指令的作用为*将内存地址为 `x22+`#fix[`sign-extend(`]`32`#fix[`)`] 的 32 位数据#fix[（读取 4 个字节），写入]寄存器 `x9`*，*内存地址是在 `x22` 寄存器的基础上加上立即数 $16$ 左移 1 位得到的*。
...
+ #quote[对代码进行重排，插入最少的NOP指令。假设寄存器 `x17` 可用来做临时寄存器。]
  - `x12` 和 `x2` 在指令序列中没有其他依赖关系，可以调整顺序，而其他指令寄存器之间依赖关系强，在不改变指令序列功能的基础上，不能调换顺序；分析发现，当调整 ```asm lw x12, 0(x2)``` 的位置时，```asm nop``` 的数量不会发生改变；同时，在指令执行中用 `x17` 替换 `x15` 或者 `x13` 作为临时寄存器，同时不增加额外指令时，```asm nop``` 的数量不会减少，替换 `x11`，`x12` 或者 `x2` 时会影响指令序列的功能。#fix[所以无法通过重排减少 ```asm nop``` 的数量]。
...
- 每个块大小为 $1$ 个字（$4$ Bytes），则偏移 $O = log_2 4 = 2$.
